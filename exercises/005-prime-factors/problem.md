...
- 文字列が渡された場合、整数に変換できる（例: `"15"` -> `15`）なら受け入れる。
 - 要素が整数でない（浮動小数点、非数文字列、オブジェクトなど）場合は、JSON-RPC エラー `-32602`（Invalid params）を返し、メッセージを `Invalid params: items must be integers >= 2 and <= 4294967295` とすること。
 - 2 未満の整数（0,1,負数）、または許容最大値（`2^32 - 1` = `4294967295`）を超える値は無効とし、同じ `-32602` を返す。
 - 最大許容値: 実装は `2^32 - 1`（`4294967295`）を上限として扱うこと。これは符号なし 32 ビット整数の最大値である。
